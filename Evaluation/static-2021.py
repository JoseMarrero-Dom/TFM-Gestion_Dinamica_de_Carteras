'''
Cartera estática de 2021, con los mismos tickers que la cartera dinámica, pero sin reequilibrar. Con esta cartera se pueden comparar los resultados de la GAN con una estrategia buy&hold.
Con una distribución uniforme de pesos.
'''
...
def parse_args():
	parser = argparse.ArgumentParser(description="Cartera estática 2021 con pesos uniformes.")
	parser.add_argument(
		"--cache_dir",
		type=str,
	...
	parser.add_argument(
		"--out_dir",
		type=str,
		default=os.path.join(os.path.dirname(__file__), "results_static_2021"),
...
def plot_equity_regimes(lr, dates, vix, model_name, out_path):
	...
	fig.suptitle(f"Cartera estática 2021–2025 — {model_name}", fontsize=12, fontweight="bold")
...
def plot_portfolio_boxplots_by_year(weights, dates, asset_labels, model_name, out_dir):
	...
	plot_portfolio_boxplot(
		...
		os.path.join(out_dir, f"static_{model_name}_cartera_global.png"),
	)
	for yr in sorted(set(dates.year)):
		...
		plot_portfolio_boxplot(
			...
			os.path.join(out_dir, f"static_{model_name}_cartera_{yr}.png"),
		)
...
def main():
	...
	initial_weights = np.full(len(ASSET_NAMES), 1.0 / len(ASSET_NAMES), dtype=np.float32)
	...
	plot_equity_regimes(lr, dates_lr, vix_lr, "Cartera estática 2021", os.path.join(args.out_dir, "static_2021_equity.png"))
	plot_metrics_table(rows, "Métricas cartera estática 2021–2025", os.path.join(args.out_dir, "static_2021_tabla.png"))
	plot_portfolio_boxplots_by_year(weights, dates_lr, ASSET_NAMES, "Cartera estática 2021", args.out_dir)
