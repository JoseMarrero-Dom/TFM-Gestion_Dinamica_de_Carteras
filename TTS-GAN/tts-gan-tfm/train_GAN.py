...
#                 raise NotImplementedError('{} unknown inital type'.format(args.init_type))
    ...
    if args.dataset.lower() == "portfolio":
        ...
        if seq_len % args.patch_size != 0:
            if seq_len % 15 == 0:
                print("patch_size does not divide seq_len; using patch_size=15")
                args.patch_size = 15
            else:
                raise ValueError("seq_len must be divisible by patch_size")
    else:
        asset_list = None
        args.channels = args.channels or 3
        seq_len = 150

    # import network
    gen_net = Generator(seq_len=seq_len, patch_size=args.patch_size, channels=args.channels, latent_dim=args.latent_dim)
    print(gen_net)
    dis_net = Discriminator(in_channels=args.channels, patch_size=args.patch_size, seq_length=seq_len)
    print(dis_net)
...
#     train_sampler = dataset.train_sampler
    ...
    if args.dataset.lower() == "portfolio":
        ...
        test_set = dl.portfolio_load_dataset(
            data_mode="Test",
            assets=asset_list,
            window_length=args.window_length,
            stride=args.stride,
        ...
        raise ValueError("Unsupported dataset. Use --dataset portfolio.")
...
# Add module in model saving code exp'gen_net.module.state_dict()' to solve the model loading unpaired name problem
        def _state_dict(model): # return model.state_dict() if not hasattr(model, "module") else model.module.state_dict() (no hay DataParallel/DistributedDataParallel)
        ...
        save_checkpoint({
            ...
            "gen_state_dict": _state_dict(gen_net),
            "dis_state_dict": _state_dict(dis_net),
            "avg_gen_state_dict": _state_dict(avg_gen_net),
...
def gen_plot(gen_net, epoch, class_name, latent_dim):
    device = next(gen_net.parameters()).device
    synthetic_data = []
    for _ in range(10):
        fake_noise = torch.randn(1, latent_dim, device=device)
    ...
    for i in range(2):
        for j in range(5):
            channels = synthetic_data[0].shape[1]
            for c in range(channels):
                axs[i, j].plot(synthetic_data[i*5+j][0][c][0][:])
    buf = io.BytesIO()
